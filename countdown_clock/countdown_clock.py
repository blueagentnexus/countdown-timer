...
class TimerWindow:
    ...
    def toggle_pause(self):
        paused = not self.cfg.get("paused", False)
        ...
        if paused:
            ...
            if self.cfg.get("mode") == "duration":
                try:
                    ...
                    self.cfg["remaining_at_pause"] = max(0, remaining)
                except Exception:
                    self.cfg["remaining_at_pause"] = 0
            else:
                target = self._target_dt()
                remaining = (target - now).total_seconds()
                self.cfg["remaining_at_pause"] = max(0, remaining)
        else:
            # Resume: shift end time (target or start_time)
            remaining = self.cfg.get("remaining_at_pause", 0)
            now = datetime.now()
            if self.cfg.get("mode") == "duration":
                duration = self.cfg.get("duration", 0)
                new_start = now - timedelta(seconds=(duration - remaining))
                self.cfg["start_time"] = new_start.replace(microsecond=0).isoformat()
            else:
                new_target = now + timedelta(seconds=remaining)
                self.cfg["target"] = new_target.replace(microsecond=0).isoformat()
    ...
    def _tick(self, force=False):
        ...
        if paused:
            remaining_sec = self.cfg.get("remaining_at_pause", 0)
            total_seconds = remaining_sec
            target = now + timedelta(seconds=remaining_sec)
        elif self.cfg.get("mode") == "duration":
            try:
                start = datetime.fromisoformat(self.cfg["start_time"])
                duration = self.cfg.get("duration", 0)
                target = start + timedelta(seconds=duration)
        ...
        if total_seconds <= 0:
            ...
            # Alarm logic: only fire if timer JUST hit zero and wasn't already 0 at launch
            if not self._alarm_fired:
                if not getattr(self, "_startup_check", False):
            ...
            if self.cfg.get("mode") == "duration":
                self.sub.configure(text=f"Timer ends at: {target.strftime('%I:%M:%S %p')}{pause_str}")
            else:
                self.sub.configure(text=f"Target: {target.strftime('%Y-%m-%d %I:%M %p')}{pause_str}")
